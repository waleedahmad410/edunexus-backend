...
export class Migration20260511092500AuditLogs extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `alter table "audit_logs" add constraint "audit_logs_school_id_foreign" foreign key ("school_id") references "schools" ("id") on update cascade on delete set null;`,
    );
    this.addSql(
      `alter table "audit_logs" add constraint "audit_logs_branch_id_foreign" foreign key ("branch_id") references "branches" ("id") on update cascade on delete set null;`,
    );
    this.addSql(
