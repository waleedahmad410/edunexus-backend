...
export class Migration20260511090400Roles extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `create unique index "roles_school_id_branch_id_code_unique" on "roles" ("school_id", "branch_id", "code");`,
    );
    this.addSql(
      `create index "roles_school_id_index" on "roles" ("school_id");`,
    );
    this.addSql(
      `create index "roles_branch_id_index" on "roles" ("branch_id");`,
    );
    ...
    this.addSql(
      `alter table "roles" add constraint "roles_school_id_foreign" foreign key ("school_id") references "schools" ("id") on update cascade on delete set null;`,
    );
    this.addSql(
      `alter table "roles" add constraint "roles_branch_id_foreign" foreign key ("branch_id") references "branches" ("id") on update cascade on delete set null;`,
    );
  }
