...
export class Migration20260511090000Schools extends Migration {
  override async up(): Promise<void> {
    this.addSql(`create table "schools" ("id" uuid not null, "name" varchar(150) not null, "code" varchar(50) not null, "email" varchar(150) null, "phone" varchar(20) null, "address" text null, "logo_url" varchar(500) null, "status" varchar(30) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, constraint "schools_pkey" primary key ("id"));`);
    this.addSql(`create unique index "schools_code_unique" on "schools" ("code");`);
