...
export class Migration20260511091900Teachers extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(`create unique index "teachers_staff_id_unique" on "teachers" ("staff_id");`);
    this.addSql(`create index "teachers_staff_id_index" on "teachers" ("staff_id");`);
    this.addSql(`create index "teachers_status_index" on "teachers" ("status");`);
    this.addSql(`alter table "teachers" add constraint "teachers_staff_id_foreign" foreign key ("staff_id") references "staff" ("id") on update cascade;`);
  }
